  ...
  const handleBulkDownload = async () => {
    ...
    try {
      for (const checkId of selectedChecks) {
        const check = highPriorityChecks.find((c) => c._id === checkId);
        if (check) {
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold mb-6 text-center">
        High Priority Checks Overview
      </h1>
      {highPriorityChecks.length === 0 ? (
        <p className="text-gray-400 text-center">
          No high priority checks available.
        </p>
      ) : (
          ...
          <div className="bg-gray-900 p-6 rounded-lg">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-gray-800 border border-gray-700 rounded-lg">
                ...
                <tbody>
                  {highPriorityChecks.map((check, index) => (
                    <motion.tr
                      key={check._id}
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
