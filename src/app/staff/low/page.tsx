...
export default function LowPriorityChecksPage() {
  const [lowPriorityChecks, setLowPriorityChecks] = useState<Check[]>([]);
  ...
  useEffect(() => {
    const fetchLowPriorityChecks = async () => {
      try {
        const response = await fetch(
          `${serverURL}/staff/checks/low-priority`,
          {
            headers: {
        ...
        if (response.ok) {
          ...
          if (data.checks.length > 0) {
            ...
            setLowPriorityChecks(checksWithStoredFileName);
            fetchExpectedFiles(checksWithStoredFileName[0].fileId);
          }
        } else {
          toast.error('Failed to fetch low priority checks.');
        }
      } catch (error) {
        console.error('Failed to fetch low priority checks:', error);
        toast.error('Failed to fetch low priority checks.');
      }
    };

    if (localStorage.getItem('token')) {
      fetchLowPriorityChecks();
    }
  }, []);
  ...
  const confirmUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedChecks = lowPriorityChecks.filter((_, index) => index !== currentCheckIndex);
        setLowPriorityChecks(updatedChecks);
        setFiles([]);
  ...
  const handleBulkDownload = async () => {
    ...
    try {
      for (const checkId of selectedChecks) {
        const check = lowPriorityChecks.find((c) => c._id === checkId);
        if (check) {
  ...
  const handleCheckSelection = (index: number) => {
    ...
    fetchExpectedFiles(lowPriorityChecks[index].fileId);
  };
  ...
  const calculateProgress = (deliveryTime: string) => {
    const now = new Date();
    const delivery = new Date(deliveryTime);
    const total = 72 * 60 * 60 * 1000; // 72 hours total time for low priority
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-bold mb-6 text-center">
        Low Priority Checks Overview
      </h1>
      {lowPriorityChecks.length === 0 ? (
        <p className="text-gray-400 text-center">
          No low priority checks available.
        </p>
      ) : (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="text-gray-300">
                  <strong>ID:</strong> {lowPriorityChecks[currentCheckIndex]._id}
                </div>
                <div className="text-gray-300 flex items-center">
                  <strong>File ID:</strong>&nbsp;
                  {lowPriorityChecks[currentCheckIndex].fileId}
                  <Button
                    ...
                    onClick={() => handleDownload(lowPriorityChecks[currentCheckIndex].fileId)}
                ...
                <div className="text-gray-300">
                  <strong>Stored File:</strong> {lowPriorityChecks[currentCheckIndex].storedFileName}
                </div>
                <div className="text-gray-300">
                  <strong>File Size:</strong> {formatFileSize(lowPriorityChecks[currentCheckIndex].fileSize)}
                </div>
                <div className="text-gray-300">
                  <strong>Delivery Time:</strong> {new Date(
                    lowPriorityChecks[currentCheckIndex].deliveryTime,
                  ).toLocaleString()}
                </div>
                <div className="text-gray-300">
          ...
          <div className="bg-gray-900 p-6 rounded-lg">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-gray-800 border border-gray-700 rounded-lg">
                ...
                <tbody>
                  {lowPriorityChecks.map((check, index) => (
                    <motion.tr
                      key={check._id}
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
